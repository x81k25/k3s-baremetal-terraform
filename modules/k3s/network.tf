...
resource "null_resource" "network_validation" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      if ! ip link show ${var.k3s_config.network_config.interface_name} >/dev/null 2>&1; then
        echo "Error: Network interface ${var.k3s_config.network_config.interface_name} does not exist"
        exit 1
      fi

      # Validate IP address format
      if ! echo ${var.k3s_config.network_config.host_ip} | grep -P '^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$' >/dev/null; then
        echo "Error: Invalid IP address format: ${var.k3s_config.network_config.host_ip}"
        exit 1
      fi

      # Check if IP is already in use (if not the current machine's IP)
      if ! ip addr | grep ${var.k3s_config.network_config.host_ip} >/dev/null; then
        if ping -c 1 -W 1 ${var.k3s_config.network_config.host_ip} >/dev/null 2>&1; then
          echo "Warning: IP address ${var.k3s_config.network_config.host_ip} might be in use"
        fi
      fi
    EOT
  }
